...
### 목표
* 인증 : 데이터가 귀속되는 소스에서 제공되는지 확인
* 무결성 : 전송 중 데이터가 변경되지 않았는지 확인

### 대칭 암호화
* 암호화 과정
 1. 비밀 키가 송신 및 수신 컴퓨터에 알려진다.
 2. 송신 컴퓨터는 예약된 암호화 알고리즘과 비밀 키를 사용해 데이터를 암호화
 3. 암호화된 텍스트는 대상 컴퓨터에 전송
 4. 수신 컴퓨터는 상응하는 암호화 알고리즘을 사용해 데이터를 해독

### 비대칭 암호화
* 데이터를 암호화하는 키와 해독하는 키가 다르기 때문에 비대칭이라고 부른다.
* 암호화 과정
 1. 컴퓨터 A가 컴퓨터 B와 연결을 설정
 2. 컴퓨터 B가 컴퓨터 A에게 공개 키를 전송
 3. 컴퓨터 A가 컴퓨터 B로부터 받은 공개 키로 데이터를 암호화하고 데이터를 전송하며, 컴퓨터 B로부터 받은 공개 키는 이후 참고를 위해 컴퓨터 A에 저장
 4. 컴퓨터 B는 데이터를 받아서 개인 키를 사용해 해독

### 디지털 서명
* 데이터가 해당 출처에서 왔으며 전송 경로를 따라 데이터가 변하지 않았다는 것을 보장하는 방법
* 디지털 서명 과정
 1. 컴퓨터 A가 컴퓨터 B에 문서를 보내고 싶을 때, 컴퓨터 A는 문서의 내용을 검증하는데 필요한 정보와 함께 작은 데이터 조각을 생성하고, 문서 비트에 일부 수학적 게싼을 수행해 값을 도출한다. (인증자는 메시지의 진위성을 검증하기 위한 타임 스탬프 값이나 첨부된 메시지와 검증자를 연관시킬 수 있는 다른 매개변수와 같은 다른 유용한 정보를 포함할 수도 있다.)
 2. 컴퓨터 A는 개인 키를 사용해 인증자를 암호화하여, 인증자를 문서에 첨부해 컴퓨터 B에 문서를 전송
 3. 컴퓨터 B는 데이터를 받아 컴퓨터 A의 공개 키를 사용해 인증자를 해독하고, 인증자 내의 정보를 통해 컴퓨터 B는 데이터가 전송 중에 변하지 않았음을 검증한다.
 4. 컴퓨터 B의 공개 키를 사용해 데이터를 해독할 수 있다는 사실은 데이터가 컴퓨터 A의 개인 키를 사용해 암호화 되었음을 증명하며, 데이터가 컴퓨터 A에서 왔음을 보장한다.

### 디지털 인증서
* 암호화된 복사 공개 키
* 인증 기관 : 통신하고자 하는 두 당사자와 안전한 관계를 맺고 있는 타사 인증 서버
* 과정
 1. 사용자 B가 공개 키의 복사본을 보안 통신을 통해 인증 서버로 전송
 2. 인증 서버는 다른 키를 사용해 사용자 B의 공개 키를 암호화 ( 새롭게 암호화된 패키지를 인증서라 하며, 인증서버의 디지털 서명이 인증서와 함께 포함된다. )
 3. 인증 서버는 사용자 B에게 인증서를 반환
 4. 사용자 A는 사용자 B의 공개 키가 필요하다. 컴퓨터 A는 컴퓨터 B에게 사용자 B의 인증서 복사본을 요구한다.
 5. 컴퓨터 A는 인증 서버의 보안 통신을 통해 인증서를 암호화하는데 사용될 키의 복사본을 획득
 6. 컴퓨터 A는 인증 서버로 부터 얻은 키를 사용해 인증서를 해독하고 사용자 B의 공개 키를 추출
 7. 컴퓨터 A는 인증서가 진짜라는 것을 보장하기 위해 인증 서버의 디지털 서명을 확인

### SSL
* 웹 통신 보호를 위해 넷스케이프에서 처음 소개한 TCP/IP 보안 프로토콜 집합
* 전송계층에 있는 소켓과 소켓을 통해 네트워크에 접근하는 애플리케이션간 보안 계층을 제공하는 것
* TLS 레코드 프로토콜 : TCP 접근을 위한 표준 기반
* TLS 핸드셰이크 프로토콜 : 인증을 다루며 연결 설정을 협상하는 것
* TLS 변경 사이퍼 스펫 프로토콜 : 협상된 암호화 매개변수 변경 같은 전환을 알림
* TLS 알림 프로토콜 : 알림을 보냄

### IPsec
* TCP/IP 네트워크에 사용하는 대체 보안 프로토콜 시스템
* TCP/IP 프로토콜 스택 내부 전송 계층 밑에서 작동
* 전송 계층 밑에서 구현되었기에 전송 계층 위에서 작동하는 애플리케이션은 보안 시스템을 알 필요가 없다.
* 모드
 1. 전송모드 : IP 패킷의 페이로드를 위한 암호화를 제 공, 페이로드는 전송을 위해 일반적은 IP 패킷에 포함
 2. 터널모드 : 전체 IP 패킷을 암호화, 암호화된 패킷은 다른 외부 패킷에 페이로드로 포함

### 켈베로스
* 위험한 네트워크에 안전한 접근을 지원하기 위해 설계된 네트워크 기반 인증 및 접근 제어 시스템
* 통신 호스트에 키를 분배하고 서비스에 접근을 요청하는 사용자의 자격 증명을 위한 체계적인 프로세스를 제공
* 과정
 1. 클라이언트가 서버 A의 서비스에 접근할 때, 클라이언트는 서버 A의 서비스 접근 요청을 KDC에 보낸다.
 2. KDC는 클라이언트와 서버 A간 통신 암호화를 위해 사용된 세션 키를 생성
 3. KDC는 세션 티켓을 생성 ( 세션 티켓은 세션 키의 복사본을 포함하며, 타임스탬프 정보 및 클라이언트 보안 설정과 같은 접근을 요청하는 클라이언트에 대한 정보를 포함 )
 4. KDC는 서버 A의 장기 키를 사용해 세션을 암호화
 5. KDC는 암호화된 세션 티켓, 세션 키 복사본 및 클라이언트를 위한 다른 응답 매개변수를 번들링하고 클라이언트의 키를 사용해 전체 패키지를 암호화
 6. 클라이언트는 KDC로부터 응답을 수신하고 해독하여 서버 A와 통신하는 데 필요한 세션 키를 획득
 7. 클라이언트는 서버 A에 접근 요청을 보낸다. ( 이 요청은 세션 티켓과 인증자를 포함 )
 8. 서버 A는 요청을 수신하고 장기 키를 사용해 세션 티켓을 해독하여 세션키를 추출하여 세션 키로 인증자를 해독
 9. 서버 A는 세션 티켓에 포함된 정보가 인증자의 정보와 일치하는지 확인하여 일치한다면 서비스 접근이 주어진다.
 10. 별도의 마지막 단계로 만약 클라이언트가 서버 A의 자격 증명을 확인하고 싶다면, 서버 A는 세션 키와 함께 인증자를 암호화하고 클라이언트로 해당 인증자를 반환

### 퀴즈
1. 엘런은 윈도 XP 컴퓨터에서 여러 레거시 네트워크 애플리케이션을 만들 방법을 찾아야 합니다. 이 오래된 앱을 사용해서 통신할 때 기밀성을 제공하도록 지시받았습니다. TLS를 사용해야 할까요? IPsec를 사용해야 할까요?
 * IPsec, IPsec는 전송 계층 밑에서 구현되었기에 전송 계층 위에서 작동하는 애플리케이션은 보안 시스템을 알 필요가 없다.
2. 만약 침입자가 켈베로스 클라이언트를 속여 세션 티켓을 잘못된 서버로 보내도록 하면 어떻게 될까요?
 * 침입자가 서버의 장기 키를 알지 못한다면 아무 일도 일어나지 않는다, 세션 티켓은 서버의 장기 키로 암호화 된 것으로 침입자가 서버의 장기 키를 알 수 없다면 티켓을 해독할 수 없음.
3. 세션 쿠기과 영구 쿠키의 차이점은 무엇입니까?
 * 세션 쿠키 : 메모리에 존재하고, 세션 기간 동안만 남아있음
 * 영구 쿠키 : 세션 동안 살아있고 지정된 만료 날짜까지 남아 있음
4. 웹 브라우저에서 추적 금지를 활성화했는데 광고 회사가 계속해서 나를 추적하는 것 같습니다. 내가 잘못한 것은 무엇일까요?
 * 잘못 없음, 추적 금지 요청을 준수할지 여부는 자발적임
